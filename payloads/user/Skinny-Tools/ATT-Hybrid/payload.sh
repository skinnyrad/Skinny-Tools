#!/bin/sh
# ATT-Hybrid — start the v28 orchestrator in 'hybrid' mode (both
# Passpoint and open BSSs up simultaneously) + live display helper.
# Live display pushes every RADIUS hit to the Pager UI as it arrives.
...
if [ -z "$LATEST_RADIUS" ] || [ -z "$LATEST_RUN" ]; then
    echo "[att-hybrid] orchestrator didn't create logs in 30s; check ATT-Status"
